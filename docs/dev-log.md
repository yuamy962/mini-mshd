...
#### P1（建议完成）✅ 已完成
- [x] 对比卡片模式（normal / compare）：分享卡片弹窗增加模式切换，对比模式展示"差回答 vs 好回答"
- [x] 分享引导弹窗：首次生成后 1.5 秒自动弹出，引导用户分享
- [x] 再生成按钮文案随机（"换个问题再试一次" / "再刷一个更好的回答"）

#### P2（可后做）✅ 已完成
- [x] 面试雷点提醒样式优化：红色渐变背景 + 装饰角 + 左边框强调 + 文字加粗
- [x] 结果页加载骨架屏：带动画脉冲效果的占位骨架，提升等待体验
...
- **代码预留**：`pages/index/index.js` 中 `onWatchAd` 方法已包含完整的激励视频广告接入代码（当前注释状态）
- **启用步骤**：
  1. 开通流量主
  2. 创建「激励式视频」广告位，获取 `adUnitId`
  3. 将 `onWatchAd` 中的模拟模式注释掉，取消注释真实广告模式
  4. 填入 `adUnitId`
- **云函数**：`quota` 的 `watchAd` 逻辑已就绪，无需改动
...
| 2026-05-01 | v0.6 | 完成 v1.3 P1 + P2 功能开发 | Kimi |
